...
from rosbag_parser import nedTime
...
def main():
	...
	filename = 'successful4.bag'
	bag = rosbag.Bag('../../data/outdoor/flights_0130/' + filename)
	time_offset = 39.5
	...
	data_type = 'outdoor'
	# data_type = 'sim'
	odom, hlc = get_data(data, bag, data_type, time_offset)
	get_north_data(odom, hlc)
	get_east_data(odom, hlc)
	get_height_data(odom, hlc)


def get_data(data, bag, data_type, time_offset):

	if data_type == 'mocap_master_branch':
		
		odom = data.get_boat_odom(bag)
		...
		odom = data.get_boat_odom(bag)
		odom.time = np.array(odom.time) - odom.time[0] + time_offset
		hlc = data.get_high_level_command(bag)
		hlc.time = np.array(hlc.time) - hlc.time[0]
